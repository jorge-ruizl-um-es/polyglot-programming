...
Extensión del programa de ordenamiento --> 
    ...
    Añado la opción -f para no distinguir minusculas y mayusculas. Para implementarla, si se detecta, se convertirán todas 
    las palabras guardadas en lineptr a minusculas ANTES de la ordenación
*/
...
void qsort(char *lineptr[], int left, int right, 
            int (*comp) (void *, void *));      // se le pasa un puntero a la funcion apropiada
...
void lower_all(char *[], int);     // funcion para poner todas las lineas de una lista de lineas en minuscula

// ordenar lineas de entrada
int main(int argc, char *argv[]) {
    int nlines;
    int numeric = 0, reversed = 0, minus = 0;
    char c;
    ...
    if (argc != 0)
        printf("Uso: sort -r -n -f\n");
    else if (minus && numeric)
        printf("No tiene sentido usar las opciones 'n' y 'f' a la vez\n");
    else {
        if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {
            if (minus)
                lower_all(lineptr, nlines);
            qsort((char **) lineptr, 0, nlines-1, (int (*) (void*, void*)) (numeric ? numcmp : strcmp));
            if (reversed)
...
// funcion para pasar una cadena a minusculas
