...
int main(int argc, char *argv[]) {
    ...
    while (scanf("%s", word) != EOF) {  // si leemos palabras con scanf, habría que meter las comprobaciones para detectar 
        // comentarios, comillas... Estas estaban implícitas en al función getword. 
        if (isalpha(word[0]) && binsearch(word, types, NTYPES) >= 0) {
            char new_word[MAXWORD];
            if (scanf("%s", new_word) != EOF) {
                char prefix[MAXWORD];
